...
class VarRandomizer(ast.NodeTransformer):
    ...
    def __init__(self):
        ...
        self.homoglyphs = ['а', 'с', 'е', 'о', 'р', 'х', 'у', 'і', 'ј', 'ѕ']
        random.seed()

    def _rand(self, n=16):
        return random.choice(self.homoglyphs) + ''.join(random.choices(self.homoglyphs, k=n-1))
    ...
    def _should_rename(self, name):
        ...
        if name in self.imported:
            return False
        if name in ('__name__', '__file__', 'self', 'cls', '__init__', '__main__'):
            return False
    ...
    def visit_ClassDef(self, node):
        # We don't rename class name here, class_randomizer does that.
        # But we create a new scope for the class body.
        old_scope = self.scope
        new_scope = Scope(parent=old_scope)
        self.scope = new_scope
        self.generic_visit(node)
